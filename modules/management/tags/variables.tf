...
variable "tag_binding" {
  description = <<-EOT
  A LocationTagBinding represents a connection between a TagValue and a non-global target such as 
  a Cloud Run Service or Compute Instance. Once a LocationTagBinding is created, 
  ...
  Input is a list, ensuring multiple parents with corresponding location can be specified.
  EOT
  type = list(object({
    parent   = string
    location = string
