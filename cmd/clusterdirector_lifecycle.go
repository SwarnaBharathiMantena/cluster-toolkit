...
func buildSpecFromDeployment(deplRoot string, bp config.Blueprint, include func(config.GroupName) bool) (cdapi.Spec, bool) {
	...
	return cdapi.Spec{
		...
		ClusterName:    cdapi.SanitizeClusterID(deploymentName),
...
func waitClusterDirectorOp(ctx context.Context, client *cdapi.Client, op *cdapi.Operation) error {
	if op != nil && op.Name != "" && !op.Done {
		logging.Info("Waiting for Cluster Director operation %s (this typically takes 1-3 minutes)...", op.Name)
	}
	_, err := client.WaitForOperation(ctx, op, cdPollInterval, cdWaitTimeout)
	return err
}
