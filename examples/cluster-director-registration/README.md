...
| Value | Source |
| --- | --- |
| Cluster ID | `deployment_name`, sanitized to RFC-1034 |
...
The cluster ID is the deployment name.

```shell
./gcluster cluster-director describe \
  --project YOUR_PROJECT_ID --region us-central1 \
  --cluster-name ctk-cluster-director-demo

gcloud compute instances list --project YOUR_PROJECT_ID \
  --filter="labels.ghpc_deployment:ctk-cluster-director-demo"
```

## Destroy

```shell
./gcluster destroy ctk-cluster-director-demo --auto-approve
```
...
```shell
./gcluster destroy ctk-cluster-director-demo --only compute --auto-approve
```
