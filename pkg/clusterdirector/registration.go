...
func (s Spec) Validate() error {
	...
	if !rfc1034.MatchString(s.ClusterName) {
		return fmt.Errorf("cluster name %q must start with a letter and contain only lower-case letters, digits and hyphens (at most %d characters)", s.ClusterName, maxKeyLength)
	}
